...
# Получаем ветку из параметра или используем main по умолчанию
BRANCH=${1:-main}

echo "🔄 Zero-downtime обновление DocFinder v2 (ветка: $BRANCH)..."
echo "=========================================="
...
echo "📥 Получение обновлений из Git (ветка: $BRANCH)..."
...
read -p "🤔 Продолжить обновление с ветки $BRANCH? (y/n): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
# Обновление кода
echo "📥 Обновление кода с ветки $BRANCH..."
